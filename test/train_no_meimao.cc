...
int main(int argc,char ** argv)
{
    ...
    params.n_landmarks = 48;
//    params.initShape_n = 2;
    params.stages_n=7;
    params.tree_n=7;
    params.tree_depth=6;
//    params.bagging_overlap = 0.4;
    params.model_filename = "../models/hfy_no_liuhai_aug.xml";
    params.save_model = true;
    Ptr<ext::FacemarkTrain> facemark = ext::FacemarkLBF::create(params);

    std::string prefix = "";
    std::ifstream infile("/home/slam/nfs72/face/facial_points/data_for_train/after_liuhai_hand/total_result_ssd_shuffle.txt");
    if(!infile.is_open()) {
    ...
    while(!infile.eof()) {
        ...
        std::vector<cv::Point2f>::iterator iter = facial_points.begin() + 35;
