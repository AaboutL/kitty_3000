...
#ifndef ALIGNMENT_3000FPS_CV_FACEMARK_TRAIN_H
#define ALIGNMENT_3000FPS_CV_FACEMARK_TRAIN_H

#include <opencv2/opencv.hpp>
#include <opencv2/objdetect.hpp>
#include "facemark.h"
namespace ext {
    typedef bool(*FN_FaceDetector)(cv::InputArray, cv::OutputArray, void *userData);

    struct CParams {
        cv::String cascade; //!<  the face detector
        ...
        cv::Size minSize; //!< Minimum possible object size.
        cv::Size maxSize; //!< Maximum possible object size.

        CV_EXPORTS CParams(
                cv::String cascade_model,
                double sf = 1.1,
                int minN = 3,
                cv::Size minSz = cv::Size(30, 30),
                cv::Size maxSz = cv::Size()
        );
...
@endcode
*/
    bool loadDatasetList(cv::String imageList,
                                      cv::String annotationList,
                                      std::vector <cv::String> &images,
                                      std::vector <cv::String> &annotations);
...
@endcode
*/
    bool loadTrainingData(cv::String filename, std::vector <cv::String> &images,
                                       cv::OutputArray facePoints,
...
@endcode
*/
    bool loadTrainingData(cv::String imageList, cv::String groundTruth,
                                       std::vector <cv::String> &images,
                                       cv::OutputArray facePoints,
                                       float offset = 0.0f);
...
*/
    CV_EXPORTS_W bool loadTrainingData(std::vector <cv::String> filename, std::vector <std::vector<cv::Point2f>>
    &trainlandmarks, std::vector <cv::String> &trainimages);
...
*/
    bool loadFacePoints(cv::String filename, cv::OutputArray points,
                                     float offset = 0.0f);
    bool loadFacePoints(cv::String filename, cv::OutputArray points, int num_pts, char split_char=NULL, 
                        float offset = 0.0f);
...
@endcode
*/
    void drawFacemarks(cv::InputOutputArray image, cv::InputArray points,
...
*/
class FacemarkTrain : public Facemark {
public:
...
*/
    virtual bool addTrainingSample(cv::InputArray image, cv::InputArray landmarks)=0;

    virtual bool addTrainingSample(cv::InputArray image, cv::InputArray landmarks, cv::Rect& bbox)=0;
...
@endcode
*/
    virtual bool getFaces(cv::InputArray image, cv::OutputArray faces)=0;
...
#endif //ALIGNMENT_3000FPS_CV_FACEMARK_TRAIN_H
